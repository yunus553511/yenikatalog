...
class SearchService:
    ...
    def search(self, query: str, top_k: int = 5) -> List[Tuple[Profile, float, str]]:
        ...
        if dimension_results:
            ...
            # Ölçü araması sonuç bulduysa direkt döndür, embedding'e gitme
        ...
        # Eğer sorguda sayı varsa, ölçü araması yapılmış demektir
        ...
        if has_category:
            # Kategori belirtilmiş ama sonuç yok
    ...
    def _search_by_dimensions(self, query: str) -> List[Tuple[Profile, float, str]]:
        ...
        Örnekler:
        - "çap 28" veya "28 çap" → Ø=28
        - "30x30" veya "30 a 30" → A=30, B=30
        - "40x40 kutu" → A=40, B=40, kategori=KUTU
        - "kalınlık 2" → K=2
        """
        query_lower = query.lower()
        results = []
        ...
        if axb_match:
            ...
            # Kategori filtresi (öncelik sırasına dikkat!)
            category_filter = None
            if 'köşebent' in query_lower or 'kosebent' in query_lower:
                category_filter = 'KÖŞEBENT'
            elif 't profil' in query_lower or 't tipi' in query_lower or ' t ' in query_lower:
                category_filter = 'T'
            elif 'u profil' in query_lower or 'u tipi' in query_lower or ' u ' in query_lower:
                category_filter = 'U'
            elif 'kutu' in query_lower:
                category_filter = 'KUTU'
            elif 'lama' in query_lower:
                category_filter = 'LAMA'
        ...
        if aab_match and not axb_match:  # AxB yoksa
        ...
        # Kalınlık araması
        kalinlik_match = re.search(r'(?:kalınlık|kalinlik|k)\s*(\d+(?:\.\d+)?)', query_lower)
        if kalinlik_match and not results:  # Başka sonuç yoksa
            ...
            for profile in excel_service.get_profiles():
                if 'K' in profile.dimensions:
                    if abs(profile.dimensions['K'] - k_value) < 0.1:
                        dims_str = ", ".join([f"{k}={v}" for k, v in profile.dimensions.items()])
                        reason = f"Kalınlık: {profile.dimensions['K']}mm ({dims_str})"
                        results.append((profile, 0.8, reason))
        
        # Tek sayı (örn: "30 profil", "40 kutu", "30 lama")
        ...
        if single_num_match and not results:
            ...
            # Kategori filtresi
            category_filter = None
            if 'kutu' in query_lower or 'kare' in query_lower:
                category_filter = 'KUTU'
                logger.info(f"Kare kutu profil araması: {value}x{value}")
            elif 'lama' in query_lower:
                category_filter = 'LAMA'
                logger.info(f"Lama profil araması: {value}")
            elif 'köşebent' in query_lower or 'kosebent' in query_lower:
                category_filter = 'KÖŞEBENT'
                logger.info(f"Kare köşebent araması: {value}x{value}")
            elif 'u profil' in query_lower or 'u tipi' in query_lower:
                category_filter = 'U'
                logger.info(f"Kare U profil araması: {value}x{value}")
            elif 't profil' in query_lower or 't tipi' in query_lower:
                category_filter = 'T'
                logger.info(f"Kare T profil araması: {value}x{value}")
            ...
            if category_filter:
                for profile in excel_service.get_profiles():
                    ...
                    if 'A' in profile.dimensions and 'B' in profile.dimensions:
                        if (abs(profile.dimensions['A'] - value) < 0.1 and 
                            abs(profile.dimensions['B'] - value) < 0.1):
                            reason = f"Kare profil: {value}x{value}mm"
                            if 'K' in profile.dimensions:
                                reason += f", Kalınlık: {profile.dimensions['K']}mm"
                            results.append((profile, 1.0, reason))
